...
### Чеки (54-ФЗ) — «Чеки от ЮKassa»
Готово. Счёт создаётся с составом чека (`provider_data.receipt`), настроен под **ИП на УСН «доходы» (6%)**:
- `tax_system_code = 2` (УСН доход);
- `vat_code = 1` (позиция «Без НДС» — на УСН услуги НДС не облагаются);
- `payment_subject = service`, `payment_mode = full_payment`;
- e-mail покупателя Telegram передаёт в ЮKassa сам (`need_email` + `send_email_to_provider`).

Если сменишь налоговый режим — задай секреты `TAX_SYSTEM_CODE` и/или `VAT_CODE` (перекрывают дефолты), код менять не нужно.

> ВАЖНО: тот НДС 22%, что ЮKassa показывает в кабинете, — это налог на **их комиссию**, а не на твои услуги. В чеке покупателя позиции остаются «Без НДС».

### Сколько реально уходит с платежа
Комиссий две: Оператор (ЮMoney, 2,8%) + Платёжный агрегатор (1%), сверху НДС 22% на комиссию.
- **Карты / кошельки:** ≈ 3,8% × 1,22 ≈ **4,64%** (с 1000 ₽ — ~46 ₽).
- **СБП:** ≈ 2,2% × 1,22 ≈ **2,68%** (с 1000 ₽ — ~27 ₽).

СБП почти вдвое дешевле — есть смысл в ЮKassa включить его и продвигать как основной способ.
